...
class DriverManager(Resource):
    ...
    def run_transformation_to_mysite(self, job_id):
        print('run_transformation_to_mysite')
        try:
            print("Transformation to my site")
            #driver.run_from_db(sysinfo, False)
#'python pse_driver.py --c run_from_file --wf ./script_pse/'$script'
            print("python pse_driver.py --c transform_to_mysite --job_id %s" % str(job_id))
    ...
    def update_mysite(self, job_id):
        ...
        try:
            ...
            cmd_update_mysite = "clear;python pse_driver_dev.py --c update_to_mysite --job_id {} --groupbykey".format(job_id)  
            for idx in groupby_keys:
               cmd_update_mysite += " " + str(idx)
            print(cmd_update_mysite)
    ...
    def update_targetsite(self, job_id):
        print(job_id)
        try:
            print("Upload / Update to target site")
            #for i in target_ids:
            #   target_id = i
...
#'python pse_driver.py --c run_from_file --wf ./script_pse/'$script'
    ...
    def activate_schedule(self, dag_id):
        try:
            print("airflow unpause {}".format(dag_id))
            #driver.run_from_db(sysinfo, False)
            res = subprocess.Popen("airflow unpause  %s" % str(dag_id), stdout=subprocess.PIPE, universal_newlines=True,shell=True)
            is_success = True
    ...
    def deactivate_schedule(self, dag_id):
        try:
            print("airflow pause {}".format(dag_id))
            #driver.run_from_db(sysinfo, False)
            res = subprocess.Popen("airflow pause  %s" % str(dag_id), stdout=subprocess.PIPE, universal_newlines=True,shell=True)
            is_success = True
            for stdout_line in iter(res.stdout.readline, ""):
    ...
    def add_worker(self, worker_ip, worker_port):
        try:
            ...
            print("ssh -p {} pse@{} 'cd /home/pse/PSE-engine; /home/pse/.pyenv/shims/rq worker  --url redis://141.223.197.35:63790 -n {} -w engine.pse_worker.pseWorker --job-class engine.pse_job.pseJob real_queue'".format(worker_port, worker_ip, worker_name))
        ...
        try:
            ...
            for worker in workers:
               if worker.name == worker_name:
                  print("ssh -p {} pse@{} 'kill -8 {}'".format(port, ip, worker.pid))
    ...
    def run_driver(self, wf, job_id):
        try:
            #print("python pse_driver.py --c run_from_db --wf {} --job_id {}".format(wf, job_id))

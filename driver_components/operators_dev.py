...
class BFSIterator():
  ...
  # for detail page pagination (sometim option..)
  def run0(self, rm, results):
    ...
    for (parent_task_id, parent_node_id, urls) in results.get(input_op_id, []):
      ...
      for url in urls:
        ...
        if chunk_size == max_chunk_size:
          chunk_size = 0
          num_s, num_f = self.wait(rm, running_tasks, results)
          total_num_s += num_s
          cur_time = datetime.utcnow() + time_gap 
          cur_time = cur_time.strftime('%Y-%m-%d %H:%M:%S')
  ...
  def run1(self, rm, results):
    ...
    btn_query = self.props['selected_btn_query']
    ...
    for (parent_task_id, parent_node_id, urls) in results.get(task['input'], []):
      ...
      for url in urls:
        while True:
          ...
          if chunk_size == max_chunk_size:
            chunk_size = 0
            num_s, num_failed = self.wait(rm, running_tasks, results)

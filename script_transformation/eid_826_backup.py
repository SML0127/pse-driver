...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         result['product_name'] = translator.translate(node_properties['name'], dest="ko").text
         print("")
         ...
         if option_name1 is None or 'ご注文後のお客様都合によるキャンセルや変更は出来ません' in option_name1 :
            ...
            if option_name1 is not None and option_name1 is not '-' and option_name1 is not '':
               ...
               result['option_names'].append(str(translator.translate(node_properties['option_name'], dest="en").text))
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option']):
                  node_properties['option'][idx] = str(translator.translate(node_properties['option'][idx], dest="en").text).replace('"','').replace("'","")
               variant[translator.translate(node_properties['option_name'], dest="en").text] = node_properties['option']
               print(node_properties['option'])
         ...
         node_properties['mpid'] = '0015'+str(node_properties['mpid']).zfill(6)
         ...
         refined_description_last = refined_description_last.replace('</ ', '</').replace('< ', '<').replace('<h3>','').replace('</h3>','').replace('&amp',' ').replace('↑','').replace('→','').replace('指輪のサイズ表','')
         ...
         if refined_description_last != "":
            ...
            if node_properties['description'] is not None:
               ...
               description_content += str(translator.translate(refined_description_last,src='ja', dest="ko").text).replace('</ ', '</').replace('< ', '<').replace('<h3>','').replace('</h3>','').replace('↑','').replace('→','') + "<br style='margin-top:10pt'>"

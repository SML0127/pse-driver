...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         result['product_name'] = '(Delete useless information) ' + escape(client.translate(node_properties['name'],source_language='ja',target_language='ko')['translatedText'])
         ...
         if 'item_desc' in refined_description_sale_last:
            refined_description_sale_last= re.sub(cleanr_garbagge, '', refined_description_sale_last)
         ...
         if refined_description_last != "":
            ...
            if node_properties['description'] is not None:
               desc_con = ""
               description_content += remove_br(str(client.translate(refined_description_last, source_language='ja',   target_language='ko')['translatedText']).replace('</ ', '</').replace('< ', '<').replace('<h3>','').replace('</h3>','').replace('↑','').replace('→','') + "<br>")

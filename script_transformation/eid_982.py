...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         result['product_name'] = escape(name)
         ...
         shipping_price = node_properties.get('shipping_price', "0")
         if shipping_price == None or '送料無料' in str(shipping_price) or '送料別' in str(shipping_price) :
            shipping_price = "0"
         print("3.1 shipping price=["+str(shipping_price)+"]");

         #if int(node_properties['mpid']) == 27376:
         #   price = 25080
         #   shipping_price = "0"
         #if int(node_properties['mpid']) == 26388: 
         #   price = 27280
         #   shipping_price = "0"
         #if int(node_properties['mpid']) == 27275:
         #   price = 12000
         #   shipping_price = "0"
         ...
         refined_description_last = refined_description_last.replace('</ ', '</').replace('< ', '<').replace('&amp',' ').replace('↑','').replace('→','').replace('指輪のサイズ表','').replace('<br>',' <br> ').replace('<b>','').replace('</b>','').replace('<h1>','').replace('</h1>','<br>').replace('<h2>','').replace('</h2>','<br>').replace('<h3>','').replace('</h3>','<br>').replace('<h4>','').replace('</h4>','<br>').replace('<h5>','').replace('</h5>','<br>').replace('</font>','')

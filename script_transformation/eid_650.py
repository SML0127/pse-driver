...
def user_defined_export(graph_mgr, node_id, node_properties):
     ...
     try:
         ...
         #result = translator.translate(node_properties['name'], dest="en").text
         print(node_properties['name'])
         #result['product_name'] = node_properties['brand'] + ' - ' + translator.translate(node_properties['name'], dest='en').text
         #result['product_name'] = node_properties['brand'] + ' - ' + name
         ...
         for charge in delivery_charge_list:
             if weight > charge[0] and weight <= charge[1]:
                 ...
                 break
         print("2. delivery_charge(one)=["+str(delivery_charge)+"]")
         price = node_properties['price']
         print("3. price(yen)=["+str(price)+"]")
         
         shipping_price = node_properties['shipping_price']
         if '送料無料' in str(shipping_price):
            shipping_price = "0"
         print("3.1 shipping price(yen)=["+str(shipping_price)+"]");
         ...
         if str(shipping_price) != 'None':
            ...
            print("3.4 price + shipping = sum(yen) => ["+str(price)+"] + ["+str(shipping_price)+"] = ["+str(amount)+"]");
         ...
         if supplyPrice == delivery_charge:
            ...
            raise
         print("supply price(dollar)= "+str(supplyPrice)+"")
         ...
         if option_name1 is None and option_name2 is None:
            result['has_option'] = 'F'
         elif option_name1 is '' or option_name2 is '':
            raise 
         else:
            result['has_option'] = 'T'
            result['option_names'] = []
            if option_name1 is not None and option_name1 is not '-':
               variant = {}
               #print(option_name1)
               #print(node_properties['option1'])
               #result['option_names'].append(str(option_name1))
               result['option_names'].append(str(translator.translate(option_name1, dest="en").text))
               print(result['option_names'])
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option1']):
                  node_properties['option1'][idx] = str(translator.translate(node_properties['option1'][idx], dest="en").text)
               print(node_properties['option1'])
               variant[str(translator.translate(option_name1, dest="en").text)] = node_properties['option1']
               variants.append(variant)
            if option_name2 is not None and option_name2 is not '-':
               variant = {}
               #print(option_name2)
               #print(node_properties['option2'])
               #result['option_names'].append(str(option_name2))
               result['option_names'].append(str(translator.translate(option_name2, dest="en").text))
               print(result['option_names'])
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option2']):
                  node_properties['option2'][idx] = str(translator.translate(node_properties['option2'][idx], dest="en").text)
               print(node_properties['option2'])
               variant[str(translator.translate(option_name2, dest="en").text)] = node_properties['option2']
               variants.append(variant)
            print(result['option_names'])
         ...
         #print(node_properties['description'].replace('"',"'"))
         #print(translator.translate(node_properties['description'], dest="en").text)
         ...
         #description_content = "<div style='padding-left: 1em;'><h2>Description</h2><br>{}<br><br><br>{}</div>".format( str(translator.translate(node_properties['description'], dest="en").text), description_images)

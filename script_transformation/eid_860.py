...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         tariffRate = 0.08 if amount * yen2krw + delivery_charge >= 150 * dollar2krw else 0;
         taxRate = 0.1 if amount * yen2krw  + delivery_charge >= 150 * dollar2krw else 0;
         ...
         refined_description_last = refined_description_last.replace('</ ', '</').replace('< ', '<').replace('&amp',' ').replace('↑','').replace('→','').replace('指輪のサイズ表','').replace('<br>',' <br> ').replace('<b>','').replace('</b>','').replace('<h1>','').replace('</h1>','<br>').replace('<h2>','').replace('</h2>','<br>').replace('<h3>','').replace('</h3>','<br>').replace('<h4>','').replace('</h4>','<br>').replace('<h5>','').replace('</h5>','<br>').replace('</font>','').replace('<tr>','<tr style="border:1px solid black;">').replace('<th>','<th style="border:1px solid black;">').replace('<td>','<td style="border:1px solid black;">')

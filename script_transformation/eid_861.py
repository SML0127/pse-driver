...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         if '중고' in client.translate(node_properties['p_name'],source_language='ja',target_language='ko')['translatedText']:
             print('Used product')
             raise   
         result['product_name'] =  'KKKKKKKKKKKKK'+escape(client.translate(node_properties['p_name'],source_language='ja',target_language='ko')['translatedText'])
         ...
         if len(option_names) == 0:
            ...
            for op_n in option_names:
               variant = {}
               result['option_names'].append(escape(str(client.translate(op_n, source_language='ja', target_language='en')['translatedText'])))
               variant['stock'] = 999;
               variant[escape(str(client.translate(op_n, source_language='ja', target_language='en')['translatedText']))] = []
               for op_v in option_values[op_n]:
                  variant[escape(str(client.translate(op_n, source_language='ja', target_language='en')['translatedText']))].append(escape(str(client.translate(op_v, source_language='ja', target_language='en')['translatedText']).replace('"','').replace("'","").replace(',', ' ').replace(';', ' ').replace('#', '')))

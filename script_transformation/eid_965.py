...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         name = client.translate(node_properties['name'],source_language='ja',target_language='ko')['translatedText'] 
         if '중고' in name: 
             print('Used product')
             raise   
         result['product_name'] = node_properties['brand'] + ' - ' +escape(name)
         ...
         delivery_charge_list = node_property['shipping_fee']
         ...
         if str(shipping_price) != 'None':
            amount = float(amount) + Price.fromstring(shipping_price).amount_float;
            print("3.2 price + shipping = sum=["+str(price)+"]["+str(shipping_price)+"]["+str(amount)+"]");
         ...
         if option_name1 is None and option_name2 is None:
            ...
            if option_name1 is not None and option_name1 is not '-' and option_name1 is not '':
               ...
               result['option_names'].append(escape(str(client.translate(node_properties['option1_name'],source_language='ja',target_language='ko')['translatedText'])))
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option1']):
                  node_properties['option1'][idx] = escape(str(client.translate(node_properties['option1'][idx],source_language='ja',target_language='ko')['translatedText']).replace('"','').replace("'",""))
               variant[escape(client.translate(node_properties['option1_name'],source_language='ja',target_language='ko')['translatedText'])] = node_properties['option1']
               print(node_properties['option1'])
            ...
            if option_name2 is not None and option_name2 is not '-' and option_name2 is not '':
               ...
               result['option_names'].append(escape(str(client.translate(node_properties['option2_name'],source_language='ja',target_language='ko')['translatedText'])))
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option2']):
                  node_properties['option2'][idx] = escape(str(client.translate(node_properties['option2'][idx],source_language='ja',target_language='ko')['translatedText']).replace('"','').replace("'",""))
               variant[escape(client.translate(node_properties['option2_name'],source_language='ja',target_language='ko')['translatedText'])] = node_properties['option2']
               print(node_properties['option2'])
               variants.append(variant)

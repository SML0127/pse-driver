...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         #if node_properties.get['brand', None) is None:
         ...
         # amazon smart watch는 weight를 product_info_dict에서 수집
         shipping_weight = node_properties['product_info_dict'].get('Shipping Weight', 'none').lower() #shipping weight
         item_weight = node_properties['product_info_dict'].get('Item Weight', 'none').lower() #item weight
         if shipping_weight == 'none':
            if item_weight == 'none':
               weight = '0kg'  #should be modified**************************************
            else:
               weight = item_weight
         else:
        ...
        # get weight information
         ...
         delivery_charge = 0                          # 하드코딩한 걸로 보임 (5kg의 배송비는 테이블을 보면  35834.0 임

...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         print(node_properties['additional_info_dict'])
         print(node_properties['additional_info_dict2'])
         print(node_properties['additional_info_dict3'])
         temp_weight = node_properties['additional_info_dict'].get('Shipping Weight:', None)
         temp_weight2 = node_properties['additional_info_dict2'].get('Shipping Weight:', None)
         temp_weight3 = node_properties['additional_info_dict3'].get('Shipping Weight:', None)
         print(temp_weight, " ----- ", temp_weight2, " ------ ", temp_weight3)
         if temp_weight is not None:
             for s in str(temp_weight).lower().split(' '):
                 if 'pound' in s or 'pounds' in s or 'kg' in s or 'ounce' in s or 'g' in s:
                     weight = str(temp_weight).lower().split(' ')[2] + str(temp_weight).lower().split(' ')[3]
         elif temp_weight2 is not None:
             for s in str(temp_weight2).lower().split(' '):
                 if 'pound' in s or 'pounds' in s or 'kg' in s or 'ounce' in s or 'g' in s:
                     weight = str(temp_weight2).lower().split(' ')[2] + str(temp_weight2).lower().split(' ')[3]
         elif temp_weight3 is not None:
             for s in str(temp_weight3).lower().split(' '):
                 if 'pound' in s or 'pounds' in s or 'kg' in s or 'ounce' in s or 'g' in s:
                     weight = str(temp_weight3).lower().split(' ')[3] + str(temp_weight3).lower().split(' ')[3]

         elif temp_weight is None and temp_weight2 is None and temp_weight3 is None:
             weight = node_properties['additional_info_dict'].get('Shipping Weight', '5kg').lower()
             weight2 = node_properties['additional_info_dict2'].get('Shipping Weight', '5kg').lower()
             weight3 = node_properties['additional_info_dict3'].get('Shipping Weight', '5kg').lower()
             if((weight == weight2) and (weight != weight3)):
                 weight = weight3
             elif ((weight == weight3) and (weight != weight2)):
                 weight = weight2
         ...
         try:
             ...
             if 'kg' in str(weight):#weight.find('kg'):
                 print("kg")
                 weight = float(b)
             elif 'pound' in weight or 'pounds' in weight: # weight.find('pound'):
                 print("pound")
                 weight = float(b) * 0.453592
             elif 'ounce' in weight:#weight.found('ounce'):
                 print("ounce")
                 weight = float(b) * 0.0283495
             else: # weight.find('g') or weight.find('')
                 print("g")
         ...
         for charge in delivery_charge_list:
             print(charge[0], " < ", weight, " < ", charge[1], " => ", charge[-1])

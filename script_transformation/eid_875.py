...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         if '중고' in client.translate(node_properties['name'],source_language='ja',target_language='ko')['translatedText']:
             print('Used product')
             raise   
         result['product_name'] =  escape(client.translate(node_properties['name'],source_language='ja',target_language='ko')['translatedText'])
         ...
         option_name1 = node_properties.get('option_name', None)
         print(option_name1)
         variants = []
         if option_name1 is None or 'ご注文後のお客様都合によるキャンセルや変更は出来ません' in option_name1 :
            result['has_option'] = 'F'
         else:
            result['has_option'] = 'T'
            result['option_names'] = []
            if option_name1 is not None and option_name1 is not '-' and option_name1 is not '':
               variant = {}
               result['option_names'].append(escape(str(client.translate(node_properties['option_name'], source_language='ja', target_language='en')['translatedText'])))
               variant['stock'] = 999;
               for idx, val in enumerate(node_properties['option']):
                  node_properties['option'][idx] = escape(str(client.translate(node_properties['option'][idx], source_language='ja',   target_language='en')['translatedText']).replace('"','').replace("'","").replace(',', ' ').replace(';', ' ').replace('#', ''))
               variant[escape(client.translate(node_properties['option_name'], source_language='ja',   target_language='en')['translatedText'])] = node_properties['option']
               print(node_properties['option'])
         ...
         img_list.append('imgrc0072664233')
         for img in img_list:
         ...
         refined_description_last = refined_description_last.replace('</ ', '</').replace('< ', '<').replace('&amp',' ').replace('↑','').replace('→','').replace('指輪のサイズ表','').replace('<br>',' <br> ').replace('<b>','').replace('</b>','').replace('<h1>','').replace('</h1>','<br>').replace('<h2>','').replace('</h2>','<br>').replace('<h3>','').replace('</h3>','<br>').replace('<h4>','').replace('</h4>','<br>').replace('<h5>','').replace('</h5>','<br>').replace('</font>','').replace('<tr>','<tr style="border:1px solid black;">').replace('<th>','<th style="border:1px solid black;">').replace('<td>','<td style="border:1px solid black;">').replace('この商品の他のタイプを見る','')
         ...
         refined_description_sale_last = refined_description_sale_last.replace('</ ', '</').replace('< ', '<').replace('&amp',' ').replace('↑','').replace('→','').replace('指輪のサイズ表','').replace('<br>',' <br> ').replace('<b>','').replace('</b>','').replace('<h1>','').replace('</h1>','<br>').replace('<h2>','').replace('</h2>','<br>').replace('<h3>','').replace('</h3>','<br>').replace('<h4>','').replace('</h4>','<br>').replace('<h5>','').replace('</h5>','<br>').replace('</font>','').replace('<tr>','<tr style="border:1px solid black;">').replace('<th>','<th style="border:1px solid black;">').replace('<td>','<td style="border:1px solid black;">')
         ...
         for sub_str in ['"item_desc"','img58097420']:
         ...
         if refined_description_last != "":
            ...
            if node_properties['description'] is not None:
               desc_con = ""
               #description_content += str(client.translate(node_properties['description'],src='ja', dest="ko").text) + "<br style='margin-top:10pt'>"
               description_content += remove_delivery_string(remove_br(str(client.translate(refined_description_last, source_language='ja',   target_language='ko')['translatedText']))).replace('</ ', '</').replace('< ', '<').replace('<h3>','').replace('</h3>','').replace('↑','').replace('→','') + "<br>"
               if refined_description_sale_last != "" and '배송' not in client.translate(refined_description_sale_last, source_language='ja',   target_language='ko')['translatedText']:
               ...
               description_content += "<br><label style='font-weight:bold'>PROD번호</label>: {}<br>정보업데이트: {}<br><br><br>{}</div>".format(node_properties['mpid'],datetime.datetime.strptime(str(kor_time),"%Y-%m-%d %H:%M:%S.%f"), description_images)
            else:

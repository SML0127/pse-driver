...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         #판매가 = (원가 + (관세 + 개별소비세 + 교육세 + 부가세) + 배송비) * (1 + 마진율)
         buf = 5000# 버퍼로 최소이윤이 1만 5천원이었는데 5천원 뺀 1만원까지도 봐주겠다는 뜻
         origin_price = dollar2krw * amount
         if supplyPrice >= 2000000:
             tariff = origin_price * 0.08
             print("tariff= "+str(tariff)+"")
             individual_tax = (origin_price - 2000000) * 0.2
             print("individual tax= "+str(individual_tax)+"")
             edu_tax = individual_tax * 0.3             
             print("edu tax= "+str(edu_tax)+"")
             vat = (origin_price + tariff + individual_tax + edu_tax) * 0.1
             print("vat= "+str(vat)+"")
             retailPriceWithoutMargin = origin_price + tariff + individual_tax +edu_tax + vat 
             print("retailPriceWithoutMargin= "+str(retailPriceWithoutMargin)+"")
             final_price = (retailPriceWithoutMargin + delivery_charge) * (1.15)
             print("retailPriceWithoutMargin + margin + delivery charge = "+str(final_price)+"")
             ...
             if retailPriceWithoutMargin <= 700000:
                 margin = retailPriceWithoutMargin * 0.15
                 if margin < 50000:
                    margin = 50000
             else:
                 margin = retailPriceWithoutMargin * 0.1
                 if margin < 50000:
                    margin = 50000

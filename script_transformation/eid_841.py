...
def user_defined_export(graph_mgr, node_id, node_properties):
     try:
         ...
         result['product_name'] = 'NEWNEWNEW ' + escape(client.translate(node_properties['name'],source_language='ja',target_language='ko')['translatedText'])
         print("")
         ...
         cleantext1 = re.sub(cleanr1, '', node_properties['description'])
         ...
         refined_description_last = re.sub(cleanr8, '', cleantext7)
         ...
         refined_description_last = refined_description_last.strip().replace('<br><br><br><br><br><br><br><br><br><br>','').replace('※商品ページのサイズ表は海外サイズを日本サイズに換算した一般的なサイズとなりメーカー・商品によってはサイズが異なる場合もございます。サイズ表は参考としてご活用ください。','')
         print(refined_description_last)
         print('-----------------------------------------------------------------------------------------------------------------------')
         print(str(client.translate(refined_description_last,  source_language='ja',  target_language='ko')['translatedText']))
         print('-----------------------------------------------------------------------------------------------------------------------')
         ...
         cleantext1 = re.sub(cleanr1, '', node_properties['description_sale'])
         cleantext2 = re.sub(cleanr2, '', cleantext1)
         cleantext3 = re.sub(cleanr3, '', cleantext2)
         #cleantext4 = re.sub(cleanr4, '', cleantext3)
         cleantext5 = re.sub(cleanr5, '', cleantext3)
         ...
         refined_description_sale_last = refined_description_sale_last.strip().replace('<br><br><br><br><br><br><br><br><br><br>','').replace('※商品ページのサイズ表は海外サイズを日本サイズに換算した一般的なサイズとなりメーカー・商品によってはサイズが異なる場合もございます。サイズ表は参考としてご活用ください。','')
         print(refined_description_sale_last)
         print('-----------------------------------------------------------------------------------------------------------------------')
         print(str(client.translate(refined_description_sale_last,  source_language='ja',  target_language='ko')['translatedText']))
         ...
         if refined_description_last != "":
            ...
            if node_properties['description'] is not None:
               ...
               description_content += str(client.translate(refined_description_sale_last, source_language='ja',   target_language='ko')['translatedText']).replace('</ ', '</').replace('< ', '<').replace('<h3>','').replace('</h3>','').replace('↑','').replace('→','') + "<br style='margin-top:10pt'>"

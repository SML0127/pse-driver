...
2. Run a pse image as a container
    ...
    (mount on exist psql db) $ docker run -d --privileged -v /mnt/md0/pse/postgresql/9.5/main/:/var/lib/postgresql/9.5/main/:z --cap-add=NET_ADMIN -P -p 54320:5432 -p 5000:5000 -p 3000:3000 -p 20220:22 -p 63790:6379 --name pse-engine -it new_pse bash
...
2. The definition of the operators
    ...
    6. Expander
        ...
        2. The definition of attributes
            ...
            4. self: If it is 1 and there is no result of query, \
            it sends the current url to a BFSIterator
            5. matching_then_self: If it is 1 and there is result of query, \
            it sends the current url to a BFSIterator if there is
    7. ClickOperator
        1. json
            <pre>
            {

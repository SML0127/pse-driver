...
class GraphManager():
    ...
    def set_status_for_duplicated_data(self, job_id):
        try:
            query = 'select count(*) from job_source_view where id in (select id from job_source_view where mpid in (select mpid from job_id_and_mpid where job_id = {}) and id not in ( select min(id) from job_source_view where status != 3 group by groupby_key_sha256)) and status != 3;'.format(job_id)
            self.gp_cur.execute(query)
            result = self.gp_cur.fetchone()[0]
            self.gp_conn.commit()

            query = 'update job_source_view set status = 4 where id in (select id from job_source_view where mpid in (select mpid from job_id_and_mpid where job_id = {}) and id not in ( select min(id) from job_source_view where status != 3 group by groupby_key_sha256)) and status != 3;'.format(job_id)
            self.gp_cur.execute(query)
            self.gp_conn.commit()
    ...
    def log_expected_num_summary(self, exec_id, cnt):
        try:
            query = 'update execution set num_summary_expected_all = {} where id = {};COMMIT;'.format(cnt, exec_id)
            self.pg_cur.execute(query)
            return 
        except Exception as e:
            self.pg_conn.rollback()
    ...
    def log_expected_num_summary_success(self, exec_id, cnt):
        try:
            query = 'update execution set num_summary_expected_success = num_summary_expected_success + {} where id = {};COMMIT;'.format(cnt, exec_id)
            self.pg_cur.execute(query)
            return 
        except Exception as e:
            self.pg_conn.rollback()
    ...
    def log_expected_num_target_all(self, mt_history_id, cnt, targetsite):
        try:
            query = "update mt_history set num_expected_all = {} where id = {} and targetsite = '{}';COMMIT;".format(cnt, mt_history_id, targetsite)
            self.pg_cur.execute(query)
            return 
        except Exception as e:
            self.pg_conn.rollback()

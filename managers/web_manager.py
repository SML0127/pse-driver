...
class WebManager():
  ...
  def get_attribute_by_selenium_(self, element, attr):
    if attr == 'alltext':
      ...
      print_flushed(element.get_attribute(attr))
  ...
  def contain_text_in_page(self, txt):
    try:
      driver = self.get_cur_driver_()
      elem = driver.find_element_by_xpath("//*[normalize-space() = '{}']".format(txt))
      if len(elements) == 0: 
        return False
      else: 
        return True
  ...
  def contain_xpath_in_page(self, xpath):
    try:
      driver = self.get_cur_driver_()

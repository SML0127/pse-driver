...
class Cafe24SingleUploader(Resource):
  ...
  def upload_products_of_task_from_mpid(self, task):
    ...
    try:
      (args, mpids) = task
      #if str(args['gateway']).upper() == 'CAFE24':
      ...
      #print(node_ids)
      job_id = args['job_id'] 
      targetsite_url, gateway = self.graph_manager.get_targetsite(job_id)
      if 'selected' in args:
        for mpid in mpids:
          node_time = time.time()
          try:
        ...
        for mpid in mpids:
          ...
          try:
            ...
            #Status 0 = up to date, 1 = changed, 2 = New, 3 = Deleted 4 = Duplicated
            print('status : ', status)
            # If target site id is in db use status, otherwise alwasy upload as new item
            if gateway.upper() == 'CAFE24':

...
class BFSIterator(BaseOperator):
  ...
  def run(self, gvar):
    try:
      ...
      #gvar.web_mgr.restart(60)  
      #print('Restart chrome')
      # test code for zipcode
...
class BranchOperator(BaseOperator):
  def run(self, gvar):
    try:
      ...
      is_xpath = self.props['is_xpath']
      is_text = self.props['is_text']
      print("Condition = {}".format(condition))
      print("is xpath = {}".format(is_xpath))
      print("is text = {}".format(is_text))
      if type(is_text) != type(True): is_text = eval(is_text)
      if type(is_xpath) != type(True): is_xpath = eval(is_xpath)

      res = True      
      if is_text == True:
        res = gvar.web_mgr.contains_text_in_page(condition)
      elif is_xpath == True:
        res = gvar.web_mgr.contains_xpath_in_page(condition)

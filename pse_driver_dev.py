...
from managers.graph_manager_dev import GraphManagerDev
...
class PseDriver():
    ...
    def init(self):
        ...
        self.graph_manager = GraphManagerDev()
    ...
    def transform_to_mysite(self, args, sm_history_id):
        ...
        try:
            ...
            cur_time = cur_time.strftime('%Y-%m-%d %H:%M:%S')
            self.graph_manager.log_to_job_current_mysite_working(
                '\n{}\n[Finished] Upload {} items to mysite, Duplicate {} item.'.format(cur_time, num, duplicate_num), args.job_id)
            self.graph_manager.log_to_job_current_mysite_working(
        ...
        try:
            ...
            for node_id in node_ids:
                try:
                    ...
                    # Insert and Update   
                    self.graph_manager.insert_node_property_to_mysite_new(
    ...
    def transform_node_property_for_mysite(graph_mgr, node_id, node_properties):
        try:
            utcnow = datetime.utcnow()
            time_gap = timedelta(hours=9)
            kor_time = utcnow + time_gap
            ...
            if node_properties.get('price','') == '':
                print_flushed("Price is null :", node_properties['url'])
                return False
            result_for_source_view['stock'] = node_properties.get('stock', '999')
            result_for_source_view['shipping_price'] = node_properties.get('price','').encode('UTF-8','strict').hex()
            result_for_source_view['weight'] = node_properties.get('weight','')
            result_for_source_view['shipping_weight'] = node_properties.get('weight','')
            result_for_source_view['dimension_weight'] = node_properties.get('dimension_weight','')
            ...
            for key in sorted(node_properties.keys()):
                if key not in ['mpid', 'url', 'name', 'price', 'stock', 'images', 'option_list', 'option_matrix', 'images', 'front_image', 'brand', 'shipping_price', 'weight', 'shipping_weight', 'dimension_weight', 'source_site_product_id']:
                    result_for_desc['key'].append(key)
                    if key == 'description':
                        desc = re.sub(cleaner_dquotation, '',
                                      node_properties.get(key, ""))
